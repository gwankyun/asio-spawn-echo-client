...
error_code_t read_util(socket_t &socket, vector<char> &buffer, std::size_t &read_offset, yield_context_t &yield, Pred pred)
{
	...
	while (true)
	{
		...
		if (pred())
		{
			LOG_INFO("log", "message:{0}", buffer.data());
			return ec;
		}
	}
}
...
void session_t::run()
{
		...
		[this, self](yield_context_t yield)
	{
		...
		while (true)
		{
			...
			{
				LOG_INFO("log", "getline message:{0}", message);
				copy_n(message.begin(), message.size(), back_inserter(buffer));
				buffer.push_back('\0');
			}

			ec = write_n(*socket, buffer, buffer.size(), yield);
			if (ec)
			{
				LOG_ERROR("log", ec.message());
				return;
		...
		[&](yield_context_t yield)
	{
		...
		//while (true)
		{
			...
			session->socket->async_connect(
				endpoint_t(address_t::from_string("127.0.0.1"), 12500), yield[ec]);
